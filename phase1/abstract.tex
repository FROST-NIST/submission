...
Intuitively, unforgeability in a threshold signature setting guarantees that an adversary which corrupts the coordinator and up to $t-1$ signers \liz{This is the dishonest majority setting, but that's not the case for all threshold signature schemes.} cannot generate a valid signature \liz{This has not been defined.  Needs to be relative to $\pk$, for example.} for $\msg$ \liz{What $\msg$?} that has not been authorized by at least one honest signer. \liz{Authorized?}
