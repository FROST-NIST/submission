...
\chelsea{Intuitively, does this mean that honest parties contributions were not modified by the coordinator, which is possible for $\TSUF{3}$? }
\chenzhi{Making sure that honest parties' commitments are not modified by the coordinator is one way to achieve $\TSUF{4}$ given a $\TSUF{3}$ secure scheme, but it is not in general what $\TSUF{4}$ achieves or aims to guarantee.}
