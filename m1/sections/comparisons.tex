...
Note that FROST can be instantiated without the coordinator $\mathcal{C}$ by having parties broadcast
...
\item[Broadcast.] FROST does not rely on a broadcast channel; an adversary may send inconsistent protocol messages to honest parties, even within the same signing session.  Furthermore,
\ian{Shouldn't the authenticated channels discussion be its own
description point?  Authenticated channels are not a kind of broadcast
channel.}
authenticated channels are not assumed either, so an adversary can decide which protocol messages to send to an honest party on behalf of another honest party.
\liz{What about authenticated channels between the coordinator and signers?}
\ian{Not needed during signing for unforgeability, but needed during key generation
(recipient-authenticated and confidential for the trusted-dealer style
of key generation, for example).}

\end{description}

\noindent \textbf{Robustness.} FROST is not robust, meaning that there is no guarantee that a signing session will terminate.
\ian{In the presence of an adversary, at least?}
