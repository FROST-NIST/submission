...
However, $\TSUF{1}$ does not prevent the adversary from combining signature shares from different signing sessions. More precisely, for a $\TSUF{1}$-secure scheme, the adversary can start several signing sessions for $\msg$ but each with different signing coalitions or different second-round input, and as long as the total number of honest parties that responded in one of the signing sessions is at least $(t-k)$, the adversary might be able to compute a valid signature for $\msg$. Such an malicious behavior is prevented by $\TSUF{2}$, where each signing session is consider independently: for a signing session for $\msg$, we consider $\msg$ was signed in the session (or the session succeeded) only if at least $(t - k)$ honest parties responded in the session, and $\msg$ is considered signed as long as it was signed in one of the signing sessions.

\cite{BellareCKMTZ22} shows that FROST achieves the next level of security, $\TSUF{3}$, where the condition that a session succeeds is further strengthened. Namely, for a signing session with coalition $\coalition$ and second-round input $\{ (i, R_i, S_i)\}_{i \in \coalition}$, in additional to the above condition, it is also required that each honest signer $i$ with correct $(R_i, S_i)$, i.e., $(R_i, S_i)$ is the first-round commitment output by $i$,\footnote{Since the coordinator was corrupted, the coordinator can send arbitrary $(R_i, S_i)$ in the second round.} responded.

Further,~\cite{BellareCKMTZ22} shows that FROST is \emph{strong} unforgeable, referred to as $\TSSUF{3}$, which, analogous to the strong unforgeability of signature schemes, guarantees that the adversary cannot forge a signature $\sigma$ for $\msg$ that is not considered issued even if $\msg$ was signed in some signing sessions. Here, for each signing session, it is guaranteed that there exists a unique signature $\sigma$ that can be issued, and $\sigma$ is considered issued only if the session succeeded.
