...
\item Komlo and Goldberg introduced FROST, a two-round threshold Schnorr signature scheme, where the first round can be pre-processed. \liz{Define pre-processing.}
...
\item Bellare, Tessaro, and Zhu~\cite{BellareTZ22} proved the security of FROST and FROST2, assuming key generation is performed using a trusted key generation algorithm, under the one-more discrete logarithm assumption (OMDL) in the random oracle model. \liz{Say anything about BLS?}
